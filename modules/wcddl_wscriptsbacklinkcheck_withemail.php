...
$message='Hello, {sitename}<br />
To keep submitting to wscripts.net you must add our link.<br />
<br >
Regards,<br />
'.$core->siteurl;
...
function backlinkcheck() {
	...
	$page = $_GET['page'];
		...
		if(isset($_GET['id'])) {
			$id=$_GET['id'];
			sendemail($id,'Add our link',$message);	
		}
		...
		<tr><td>Name</td><td>Url</td><td>Backlink</td><td>Email</td><td>Select</td></tr>';
		while($got = mysql_fetch_assoc($get)) {
			...
			$html .= $bl;
			$html .= '</td>
			<td><a href="index.php?go=backlinkcheck&id='.$got['id'].'">Email</a></td>
	...
	for($i = 1; $i<=$maxpages ; $i++) 
	{
	    ...
	    {
            $html .= "<a href = 'index.php?go=squeue&page=$i'>[ $i ]</a> ";
        }
    }
	$html .= '</center>';	
...
function sendmail($to,$subject,$message) {
	global $core;
			 $search = array(
				...
				$siteinfo = mysql_fetch_assoc(mysql_query("SELECT * FROM wcddl_sites WHERE id='$to' LIMIT 1"));
				// replace $search with these
				$replace = array(
				   $siteinfo['name'],
				   $siteinfo['url'],
				);
